...
(select s.created_date,g.gold_signup_date,p.product_name,s.userid 
from product p 
inner join sales s
on p.product_id = s.product_id inner join goldusers_signup g 
on s.userid = g.userid
where s.created_date > g.gold_signup_date)b)c
where rnk =1  
order by gold_signup_date; 
...
inner join goldusers_signup g on
 ...
 (select d.*,case
  ...
  group by product_id,userid)d)e;
